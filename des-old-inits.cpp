...
*
* DESCRIPTION :
*       Discrete Element Simulations of bacteria in a rectangular chamber. The
*       spherocylindrical bacteria can move around and push against eachother
...
*       or different boilerplate strains that appear in bacteria.hpp, this
*       code will work as is (with only a few edits to make in the main 
*       function). However, if you have further customization, you will need to
*       actually dive into the code and make edits.
*
*       Here is how I compiled the code on different machines
*       
*         desktop : >> g++ -std=c++17 des.cpp -o des.o
*         niagara : >> module load gcc
                    >> g++ -std=c++17 -o des.o des.cpp -lstdc++fs
*         laptop  : >> g++-9 -std=c++17 des.cpp -o des.o -lstdc++fs
*       
*       And then running the code was as simple as:
*
*         >> ./des.o 42 42
*
* AUTHOR :    Jeremy Rothschild
*
* CONTACT : jerben.rothschild@gmail.com
*
*
*CPP*/
...
mt19937 generator(rd());      // random number generator used for cell placement
mt19937 len_generator(rd());  // used for cell lengths
mt19937 ang_generator(rd());  // used for cell angle

float ang = 0.0;      // percentage of angle that varies when cell generated
float len = 0.05;     // percentage of len difference between sister cells
float max_len = 0.1;  // precentage of max length difference from strain 
                      // average that cell has when born
...
class Environment; // initiliaze a class Environment, that we define below
...
class ABMagent
{
  ...
  // moving, growing, or splitting.
  //
  // For simplicity and the smoothness of our graphical output our agents
  // will be coded to move in the world coordinates. These are double
  //
  friend class environment;
  protected:
  public:
    Environment* environment;   // the environment the cells live in

    string label;               // cell label identifier. starts with number of 
                                // strain and then a bunch of 0s and 1s
    double radius;
    double length;
    double growth_rate;
    double max_length;
    ...
    double x;                   // coordinates of cell center (x, y)
    double y;
    double angle;               // angle of cell

    double vel_x;               // velocity of cell center
    double vel_y;
    double vel_angle;           // angular velocity of cell

    double acc_x;               // acceleration of cell center. in the current
    double acc_y;               // form of simulations, these are 0

    double acc_angle;           // angular acceleration, also 0

    double force_x_prev = 0.0;  // previous force,
    double force_y_prev = 0.0;
    double torque_prev = 0.0;

    double force_x = 0.0;       // force (force_x, force_y) applied to the
    double force_y = 0.0;       // center of the cell and the torque applied in
    double torque = 0.0;        // a single simulation step

    ABMagent* prev_ = NULL;     // these pointers are used in a linked list
    ABMagent* next_ = NULL;     // which reduces number of operations
    ABMagent
    (
...
int main (int argc, char* argv[]) {
  ...
  string datafolder = "./data";   // data folder
  int EXP_NUM = atoi(argv[1]);    // experiment number. the niagara batch file
                                  // generates many experiments using the same
                                  // parameters
  int SIM_NUM = atoi(argv[2]);    // simulation number. all experiments in the 
                                  // same simulation have the same parameters

  // filename of experiment
  string sim_name = datafolder + "/" + "c_exp_" + to_string(EXP_NUM) + "/";

  // filename of simulation parameters
  string sim_param_file = sim_name + "params.txt";

  // filename with data on 'total length' of all bacteria in each strain
  string sim_agent_file = sim_name + "sim" + to_string(SIM_NUM) + ".txt";

  // filename with all bacteria data in the simulation
  ...
  // initilialization of bacteria strains using initialize_N_strains function
  // note that the function variables are : 
  // initialize_N_strain( environment,
  //                      simulation number,
  //                      number of E. Coli eGFP strains,
  //                      number of E. Coli MCherry 1 strains,
  //                      number of E. Coli MCherry 2 strains,
  //                      number of E. Coli MCherry A22 strains,
  ...
  for (int i = 0 ; i < num_save_iter; i++)
  {
    ...
    // next line ends simulation when one species goes extinct
    // I've commented it out for multispecies simulations
    // if (fixate) {break;}
